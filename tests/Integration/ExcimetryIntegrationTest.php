...
//        }
//
//        usleep(100_000); // даём профайлеру шанс
//
//        $profiler->stop();
//
//        $log = $profiler->getLog();
...
//die;
        ...
        $excimetry = new Excimetry(['period' => 0.001, 'mode' => 'wall']);
        $excimetry->start();
        ...
        $lines = explode("\n", trim($result));
        $this->assertNotEmpty($lines, 'The result should contain at least one line');

        foreach ($lines as $line) {
            // Each line should match the pattern: stack_trace count
            $this->assertMatchesRegularExpression('/^.+\s\d+$/', $line, 'Each line should end with a count number');

            // Split the line into stack trace and count
            list($stackTrace, $count) = preg_split('/\s+/', $line, 2);

            // The stack trace should contain function/method calls separated by semicolons
            $this->assertStringContainsString(';', $stackTrace, 'Stack trace should contain semicolons separating function calls');

            // The count should be a positive integer
            $this->assertGreaterThan(0, (int)$count, 'Count should be a positive integer');
        }
    }
